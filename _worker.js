...
function 下载bat(域名,token) {
	return [
	  ...
	  `set "FILENAME=%~nx1"`,
	  ``,
	  `rem PowerShell命令读取文件的前200行内容，将内容转换为UTF8并进行base64编码`,
	  `for /f "delims=" %%i in ('powershell -command "$content = ((Get-Content -Path '%cd%/%FILENAME%' -Encoding UTF8) | Select-Object -First 200) -join [Environment]::NewLine; [convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($content))"') do set "BASE64_TEXT=%%i"`,
	  ``,
